...
| R-003 | R-002 | Extend the accepted V4 surface interpolation rule from flow-only sampling to the other groom attributes that are inherited or sampled across guide/render roots | refresh required | First H100 30k run completed and improved over R-001/V11: best test composite `32.7515` at 29k, final 30k `32.6048`. Pre-lock audit then found that the clean-flow route still ran a redundant render-root-to-guide startup backfill. The trainer now skips that backfill when guide roots come directly from the clean-flow target; the retained non-clean-flow path uses surface-local typed interpolation. The final R003 lock now requires a same-contract refresh run from zero with that skip included. | First run: `/work/anigroom-accept-line/outputs/r003_surface_interp_20260720_182227`; `docs/r003_attribute_interpolation.md` |
