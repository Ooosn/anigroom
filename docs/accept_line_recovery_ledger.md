...
| R-003 | R-002 | Extend the accepted V4 surface interpolation rule from flow-only sampling to the other groom attributes that are inherited or sampled across guide/render roots | refresh required | First H100 30k run completed and improved over R-001/V11: best test composite `32.7515` at 29k, final 30k `32.6048`. Pre-lock audit then found and fixed one active startup leak: guide controls initialized from render roots still used Euclidean KNN. The final R003 lock now requires a same-contract refresh run from zero with that fix included. | First run: `/work/anigroom-accept-line/outputs/r003_surface_interp_20260720_182227`; `docs/r003_attribute_interpolation.md` |
