...
| R-004 | R-003 | Replace render-root `target_direct` insertion with topology-local evidence parent selection and split/delete lifecycle densification | verified | Implemented as a separate interface, not a change to R003. It keeps the same statistics, clean-flow target, interpolation, losses, and A/B schedule; only `DENSIFY_PARENT_SELECTION=evidence_local_max` and the equivalent net-growth event budget change. HGC `VERIFY_ONLY=1` passed at commit `5c0abf0e45678c091ba89b5d13b0976463e84b4d` against the frozen R003 data and V4 clean-flow target. | `docs/r004_lifecycle_densification.md`; `configs/r004_evidence_localmax_0_9k.env`; `configs/r004_evidence_localmax_9k_30k.env`; HGC checkout `/home/wangyy/anigroom-r004-localmax` |
...
| 2026-07-21 | Repaired the R-004 independent-checkout path contract. | The first R004 qsub exposed that `apply_alignment_to_namespace()` overwrote explicit CLI `--data-root` and `--mesh-path` with alignment-json defaults. This was invisible in the R003 checkout because the default data existed there, but invalid for the isolated R004 checkout. The R004 launcher also now forwards the external V4 target as `CLEAN_FLOW_TARGET`. | Local `py_compile` and `bash -n` passed. This changes path precedence and launcher wiring only; lifecycle/densification code is unchanged. |
