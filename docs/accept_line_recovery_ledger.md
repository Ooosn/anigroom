...
| R-003 | R-002 | Extend the accepted V4 surface interpolation rule from flow-only sampling to the other groom attributes that are inherited or sampled across guide/render roots | refresh running | First H100 30k run completed and improved over R-001/V11: best test composite `32.7515` at 29k, final 30k `32.6048`. Pre-lock audit then found that the clean-flow route still ran a redundant render-root-to-guide startup backfill. The trainer now skips that backfill when guide roots come directly from the clean-flow target; the retained non-clean-flow path uses surface-local typed interpolation. H100 refresh job `124595795` is running from `c7b51a0` and must finish before R003 is declared locked. | First run: `/work/anigroom-accept-line/outputs/r003_surface_interp_20260720_182227`; refresh output `/home/wangyy/anigroom-r002-locked/outputs/r003_refresh_c7b51a0_20260721_010454`; `docs/r003_attribute_interpolation.md` |
| R-004 | R-003 | Replace render-root `target_direct` insertion with topology-local evidence parent selection and split/delete lifecycle densification | planned | Implemented as a separate interface, not a change to R003. It keeps the same statistics, clean-flow target, interpolation, losses, and A/B schedule; only `DENSIFY_PARENT_SELECTION=evidence_local_max` and the equivalent net-growth event budget change. Training waits for the R003 refresh lock. | `docs/r004_lifecycle_densification.md`; `configs/r004_evidence_localmax_0_9k.env`; `configs/r004_evidence_localmax_9k_30k.env` |
