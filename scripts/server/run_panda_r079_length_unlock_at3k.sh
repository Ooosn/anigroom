...
OUTPUT_DIR="$RUNTIME_ROOT/outputs/panda_r079_length_unlock_at3k_resume_4000_20260831"
...
printf '%s\n' "$$" > "$RUNTIME_ROOT/pids/r079.pid"

echo "R079_START $(date -Is) HOST=$(hostname) GPU=$CUDA_VISIBLE_DEVICES"
...
echo "R079_TESTS_START $(date -Is)"
(
  cd "$PROJECT_ROOT"
  "$PYTHON_PATH" -B -m pytest -q
) 2>&1 | tee "$RUNTIME_ROOT/logs/full_pytest.log"
echo "R079_TESTS_DONE $(date -Is)"

echo "R079_MIGRATION_START $(date -Is)"
...
echo "R079_MIGRATION_DONE $(date -Is)"
...
echo "R079_RENDER_MIGRATED_3000_START $(date -Is)"
"$PYTHON_PATH" -B \
...
echo "R079_RENDER_MIGRATED_3000_DONE $(date -Is)"
...
echo "R079_PREFLIGHT_START $(date -Is)"
run_stage1 \
  "$PREFLIGHT_CONFIG_PATH" \
  "$PREFLIGHT_OUTPUT" \
  "$PREFLIGHT_LOG" \
...
PY
echo "R079_PREFLIGHT_DONE $(date -Is)"
...
echo "R079_RENDER_4000_START $(date -Is)"
"$PYTHON_PATH" -B \
  "$PROJECT_ROOT/tools/render_white_tiger_stage1_checkpoint_views.py" \
...
echo "R079_RENDER_4000_DONE $(date -Is)"
...
echo "R079_DONE $(date -Is) CHECKPOINT=$checkpoint"
