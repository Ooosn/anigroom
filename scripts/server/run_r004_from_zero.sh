...
RUN_ID="${RUN_ID:-r004_$(date +%Y%m%d_%H%M%S)}"
...
if [[ -n "${EXPECTED_GIT_COMMIT:-}" ]]; then
  ...
  if [[ "$actual_commit" != "$EXPECTED_GIT_COMMIT" ]]; then
    echo "[r004] git commit mismatch: ${actual_commit} != ${EXPECTED_GIT_COMMIT}" >&2
    exit 2
  fi
fi
...
if ! git diff --quiet -- "${tracked_sources[@]}"; then
  echo "[r004] tracked source files are dirty; commit or revert before training" >&2
  git status --short -- "${tracked_sources[@]}" >&2
  exit 3
fi
...
check_file_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[r004] data hash mismatch: ${label}: ${actual} != ${expected}" >&2
    exit 4
  fi
  printf '%s  %s\n' "$actual" "$label"
}
...
check_directory_manifest() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[r004] directory manifest mismatch: ${label}: ${actual} != ${expected}" >&2
    exit 5
  fi
  printf '%s  %s\n' "$actual" "$label"
}
...

{
  ...
  git status --short -- "${tracked_sources[@]}"
  sha256sum "${tracked_sources[@]}"
} > "${RUN_ROOT}/source_manifest.txt"

"$PYTHON" -m compileall -q anigroom tools/train_white_tiger_stage1.py
"$PYTHON" tools/test_root_lifecycle_local_max.py

if [[ "${VERIFY_ONLY:-0}" == "1" ]]; then
  echo "[r004] VERIFY_ONLY=1; source and data contracts passed"
  exit 0
fi

echo "[r004] phase A: iteration 0 -> 9000"
PROJECT_ROOT="$PROJECT_ROOT" \
PYTHON="$PYTHON" \
DATA_ROOT="$DATA_ROOT" \
MESH_PATH="$MESH_PATH" \
CLEAN_FLOW_TARGET="$FLOW_TARGET" \
RUN_ID="${RUN_ID}_a" \
...
if [[ ! -s "$PHASE_A_CHECKPOINT" ]]; then
  echo "[r004] missing phase-A checkpoint: $PHASE_A_CHECKPOINT" >&2
  exit 6
fi

echo "[r004] phase B: iteration 9000 -> 30000"
PROJECT_ROOT="$PROJECT_ROOT" \
PYTHON="$PYTHON" \
DATA_ROOT="$DATA_ROOT" \
MESH_PATH="$MESH_PATH" \
