...
PROJECT_ROOT="${PROJECT_ROOT:?set PROJECT_ROOT to the reviewed R067 checkout}"
RUNTIME_ROOT="${RUNTIME_ROOT:?set RUNTIME_ROOT to a new R067 runtime directory}"
...
EXPECTED_COMMIT="${EXPECTED_COMMIT:?set EXPECTED_COMMIT to the reviewed R067 commit}"
RUN_ID="${RUN_ID:?set RUN_ID to a fresh R067 run id}"

PARENT_CONFIG_PATH="$PROJECT_ROOT/configs/r066_learned_curl_turns_0_30k.env"
...
if [[ -n "$(git status --porcelain=v1)" ]]; then
  echo "[r067] checkout is dirty; refusing formal execution" >&2
  git status --short >&2
  exit 2
fi
...
if grep -Eq '^CURRENT_CHECKPOINT_VERSION[[:space:]]*=[[:space:]]*[78][[:space:]]*$' "$TRAINER_PATH"; then
  fail "schema 7/8 trainer is not allowed for R067"
fi

if [[ -n "${RESUME_CHECKPOINT:-}" ]]; then
  fail "RESUME_CHECKPOINT is forbidden; R067 must start from zero"
fi
if [[ "${RESUME_OPTIMIZER:-1}" == "0" ]]; then
  fail "RESUME_OPTIMIZER=0 is forbidden; R067 has no resume path"
fi
if [[ "${STAGE1_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  fail "STAGE1_PREFLIGHT_ONLY=1 is forbidden; R067 requires formal training"
fi
if [[ "${RUN_PREFLIGHT:-1}" == "0" ]]; then
  fail "RUN_PREFLIGHT=0 is forbidden; R067 requires the full data preflight"
fi
if [[ "${RUN_BATCH_PREFLIGHT:-0}" != "0" ]]; then
  fail "reduced batch preflight is forbidden for R067"
fi
...
snapshot_config "$PARENT_CONFIG_PATH" "$CONTRACT_DIR/r066_0_30k.env"
...
delta = {
    key: {"r066": base.get(key), "r067": candidate.get(key)}
    for key in sorted(keys)
    if base.get(key) != candidate.get(key)
}
expected_delta = {
    "GUIDE_FRIZZ_RESIDUAL_SCALE": {"r066": "1.0", "r067": None},
    "SHAPE_FRIZZ_SCALE": {"r066": "1.0", "r067": None},
}
if delta != expected_delta:
    raise RuntimeError(f"R067 config delta is not exactly the two frizz removals: {delta}")
for key, expected in {
    "EXPECTED_WIDTH": "1920",
    ...
    if candidate.get(key) != expected:
        raise RuntimeError(
            f"R067 requires {key}={expected}, got {candidate.get(key)!r}"
        )
Path(sys.argv[3]).write_text(json.dumps(delta, indent=2) + "\n", encoding="utf-8")
print("R067_CONFIG_DELTA_PASS")
print("R067_FULL_RESOLUTION_0_30K_CONFIG_PASS")
PY
...
echo "[r067] host=$(hostname) commit=$actual_commit ulimit_v=$(ulimit -v)"
"$PYTHON" -m pytest -q

echo "[r067] start formal from-zero 0-30k at=$(date --iso-8601=seconds)"
env \
  -u RESUME_CHECKPOINT \
  -u RESUME_OPTIMIZER \
...
if version != 9:
    raise RuntimeError(f"R067 requires checkpoint schema 9, got {version}")
if int(checkpoint.get("iteration", -1)) != 30000:
    raise RuntimeError(
        f"R067 requires a from-zero 30k checkpoint, got iteration "
        f"{checkpoint.get('iteration')!r}"
    )
if checkpoint.get("checkpoint_kind") != "stage1_full":
    raise RuntimeError(
        f"R067 requires checkpoint_kind=stage1_full, got "
        f"{checkpoint.get('checkpoint_kind')!r}"
    )
print("R067_SCHEMA9_FROM_ZERO_30K_PASS")
PY
echo "[r067] formal run complete at=$(date --iso-8601=seconds)"
