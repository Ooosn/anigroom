...
PHASE_B_OUTPUT="${RUN_ROOT}/phase_b_v11_9k_30k"
...
27166deb7b7f95a2be55e2b7dc8bcb4e4d46905ae56fa4aba3f4cfcbb8c183ec  configs/white_tiger_stage1_local_rgb_groom_v11_appearance_lenfree_from9k.env
...
check_file_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[v11-v4] data hash mismatch: ${label}: ${actual} != ${expected}" >&2
    exit 4
  fi
  printf '%s  %s\n' "$actual" "$label"
}
...
check_directory_manifest() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[v11-v4] directory manifest mismatch: ${label}: ${actual} != ${expected}" >&2
    exit 5
  fi
  printf '%s  %s\n' "$actual" "$label"
}
...

{
  ...
  sha256sum \
    ...
    scripts/server/run_v11_v4_from_zero.sh
} > "${RUN_ROOT}/source_manifest.txt"

"$PYTHON" -m compileall -q anigroom tools/train_white_tiger_stage1.py

if [[ "${VERIFY_ONLY:-0}" == "1" ]]; then
  echo "[v11-v4] VERIFY_ONLY=1; source and data contracts passed"
  exit 0
fi

echo "[v11-v4] phase A: v4 parent route, iteration 0 -> 9000"
...
if [[ ! -s "$PHASE_A_CHECKPOINT" ]]; then
  echo "[v11-v4] missing phase-A checkpoint: $PHASE_A_CHECKPOINT" >&2
  exit 3
fi

echo "[v11-v4] phase B: exact v11 handoff, iteration 9000 -> 30000"
PROJECT_ROOT="$PROJECT_ROOT" \
PYTHON="$PYTHON" \
DATA_ROOT="$DATA_ROOT" \
MESH_PATH="$MESH_PATH" \
