...
printf '%s\n' "$$" > "$RUNTIME_ROOT/pids/r077.pid"

echo "R077_START $(date -Is) HOST=$(hostname) GPU=$CUDA_VISIBLE_DEVICES"
...
echo "R077_TESTS_START $(date -Is)"
(
  cd "$PROJECT_ROOT"
  "$PYTHON_PATH" -B -m pytest -q
)
echo "R077_TESTS_DONE $(date -Is)"
...
echo "R077_PREFLIGHT_START $(date -Is)"
run_stage1 \
...
print(json.dumps({
    ...
    "config": {
        "clean_flow_length_init_scale": config["clean_flow_length_init_scale"],
        ...
        "clean_flow_guide_length_anchor_weight": (
            config["clean_flow_guide_length_anchor_weight"]
        ),
...
echo "R077_PREFLIGHT_DONE $(date -Is)"

echo "R077_TRAIN_START $(date -Is)"
run_stage1 \
  "$OUTPUT_DIR" \
  env \
...
echo "R077_DONE $(date -Is) CHECKPOINT=$checkpoint"
