...
printf '%s\n' "$$" > "$RUNTIME_ROOT/pids/r076.pid"

echo "R076_START $(date -Is) HOST=$(hostname) GPU=$CUDA_VISIBLE_DEVICES"
...
echo "R076_TESTS_START $(date -Is)"
(
  cd "$PROJECT_ROOT"
  "$PYTHON_PATH" -B -m pytest -q
)
echo "R076_TESTS_DONE $(date -Is)"
...
echo "R076_PREFLIGHT_START $(date -Is)"
run_stage1 \
...
echo "R076_PREFLIGHT_DONE $(date -Is)"

echo "R076_TRAIN_START $(date -Is)"
run_stage1 \
  "$OUTPUT_DIR" \
  env \
...
echo "R076_DONE $(date -Is) CHECKPOINT=$checkpoint"
