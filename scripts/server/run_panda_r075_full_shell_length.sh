...
printf '%s\n' "$$" > "$RUNTIME_ROOT/pids/r075.pid"

echo "R075_START $(date -Is) HOST=$(hostname) GPU=$CUDA_VISIBLE_DEVICES"
...
echo "R075_TESTS_START $(date -Is)"
(
  cd "$PROJECT_ROOT"
  "$PYTHON_PATH" -B -m pytest -q
)
echo "R075_TESTS_DONE $(date -Is)"
...
echo "R075_PREFLIGHT_START $(date -Is)"
run_stage1 \
...
"$PYTHON_PATH" -B - "$PREFLIGHT_OUTPUT" "$CLEAN_FLOW_TARGET" "$actual_target_sha256" "$PROJECT_ROOT" <<'PY'
from __future__ import annotations
...
assert config_target.resolve() == target_path.resolve()
assert config["clean_flow_length_init_scale"] == 1.0
assert clean["clean_flow_length_init_scale"] == 1.0
...
target_length = {"available": False}
with np.load(target_path, allow_pickle=False) as target_npz:
    if "shell_h" in target_npz.files:
        shell_height = np.asarray(target_npz["shell_h"], dtype=np.float64).reshape(-1)
        if "observed" in target_npz.files:
            reliable = np.asarray(target_npz["observed"], dtype=np.bool_).reshape(-1)
        else:
            reliable = np.ones(shell_height.shape, dtype=np.bool_)
        if "shell_confidence" in target_npz.files and target_npz["shell_confidence"].ndim == 1:
            confidence = np.asarray(target_npz["shell_confidence"], dtype=np.float64).reshape(-1)
        elif "weight" in target_npz.files:
            weight = np.asarray(target_npz["weight"], dtype=np.float64).reshape(-1)
            positive = weight > 0.0
            weight_q95 = np.quantile(weight[positive], 0.95) if bool(positive.any()) else 1.0
            confidence = np.clip(weight / max(float(weight_q95), 1.0e-8), 0.0, 1.0)
        elif "direction_anchor_confidence" in target_npz.files:
            confidence = np.asarray(target_npz["direction_anchor_confidence"], dtype=np.float64).reshape(-1)
        else:
            confidence = np.ones(shell_height.shape, dtype=np.float64)
        reliable &= np.isfinite(shell_height) & (shell_height > 0.0)
        reliable &= np.isfinite(confidence) & (
            confidence >= float(config["clean_flow_length_init_min_confidence"])
        )
        if bool(reliable.any()):
            q05 = float(np.quantile(shell_height[reliable], 0.05))
            q95 = float(np.quantile(shell_height[reliable], 0.95))
            scale = float(config["clean_flow_length_init_scale"])
            target_length = {
                "available": True,
                "reliable_count": int(reliable.sum()),
                "target_reliable_shell_height_q05": q05,
                "target_reliable_shell_height_q95": q95,
                "expected_physical_range_at_scale_1.0": [q05 * scale, q95 * scale],
            }
...
echo "R075_PREFLIGHT_DONE $(date -Is)"

echo "R075_TRAIN_START $(date -Is)"
run_stage1 \
  "$OUTPUT_DIR" \
  env \
...
echo "R075_DONE $(date -Is) CHECKPOINT=$checkpoint"
