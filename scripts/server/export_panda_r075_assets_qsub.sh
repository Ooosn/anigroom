...
mapfile -t granted_devices < <(
  printf '%s\n' "$job_detail" \
    | awk '/granted_devices/ { print }' \
    | grep -oE '/dev/nvidia[0-9]+' \
    | sed -nE 's#^\/dev\/nvidia([0-9]+)$#\1#p' \
    | sort -u
)
...
local_visible_device="${visible_devices[0]}"
export CUDA_VISIBLE_DEVICES="${visible_devices[0]}"
echo "R075_ASSET_QSUB_DEVICE JOB_ID=$JOB_ID PHYSICAL_GRANTED_DEVICE=$physical_granted_device LOCAL_VISIBLE_DEVICE=$local_visible_device CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
