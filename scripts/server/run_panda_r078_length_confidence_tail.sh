...
printf '%s\n' "$$" > "$RUNTIME_ROOT/pids/r078.pid"

echo "R078_START $(date -Is) HOST=$(hostname) GPU=$CUDA_VISIBLE_DEVICES"
...
echo "R078_TESTS_START $(date -Is)"
(
  cd "$PROJECT_ROOT"
  "$PYTHON_PATH" -B -m pytest -q
)
echo "R078_TESTS_DONE $(date -Is)"
...
echo "R078_PREFLIGHT_START $(date -Is)"
run_stage1 \
...
print(json.dumps({
    ...
    "config": {
        ...
        "view_gate_length_confidence_support": (
            config["view_gate_length_confidence_support"]
        ),
        "clean_flow_guide_length_anchor_reduction": (
            config["clean_flow_guide_length_anchor_reduction"]
        ),
...
echo "R078_PREFLIGHT_DONE $(date -Is)"

echo "R078_TRAIN_START $(date -Is)"
run_stage1 \
  "$OUTPUT_DIR" \
  env \
...
echo "R078_DONE $(date -Is) CHECKPOINT=$checkpoint"
