...
PROJECT_ROOT="${PROJECT_ROOT:?set PROJECT_ROOT to the clean R054 checkout}"
RUNTIME_ROOT="${RUNTIME_ROOT:?set RUNTIME_ROOT to a new R054 runtime directory}"
...
EXPECTED_COMMIT="${EXPECTED_COMMIT:?set EXPECTED_COMMIT to the reviewed R054 commit}"

PREFLIGHT_ID=r054_primary_guide_shape_fullres_preflight_h100_20260809
RUN_ID=r054_primary_guide_shape_gaussian_residual_0_30k_h100_20260809
LABEL=r054_primary_guide_shape
...
if [[ "$actual_commit" != "$EXPECTED_COMMIT" ]]; then
  echo "[r054] commit mismatch: expected=$EXPECTED_COMMIT actual=$actual_commit" >&2
  exit 2
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "[r054] checkout is dirty; refusing formal execution" >&2
  git status --short >&2
  exit 2
fi

echo "[r054] host=$(hostname) commit=$actual_commit ulimit_v=$(ulimit -v)"
...
run_stage1() {
  ...
  if [[ -e "$output_dir" ]]; then
    echo "[r054] output already exists; refusing overwrite: $output_dir" >&2
    exit 2
  fi

  echo "[r054] start run=$run_id config=$config_name at=$(date --iso-8601=seconds)"
  ...
  echo "[r054] finish run=$run_id at=$(date --iso-8601=seconds)"
}

verify_active_path_preflight() {
  local checkpoint="$OUTPUT_ROOT/$PREFLIGHT_ID/checkpoint_000001.pt"
  [[ -s "$checkpoint" ]] || {
    echo "[r054] missing active-path preflight checkpoint: $checkpoint" >&2
    exit 2
  }
...
for key in (
    ...
    if float(checkpoint[key]) < 0.999:
        raise RuntimeError(f"R054 active-path preflight left {key} gated: {checkpoint[key]}")
...
for group, names in zip(optimizer["param_groups"], group_names, strict=True):
    for parameter_id, name in zip(group["params"], names, strict=True):
        if name in forbidden:
            raise RuntimeError(f"R054 optimizer still owns forbidden parameter: {name}")
        if name not in required:
            continue
        state = optimizer["state"].get(parameter_id, {})
        exp_avg = state.get("exp_avg")
        if exp_avg is None or not bool(torch.isfinite(exp_avg).all()):
            raise RuntimeError(f"R054 active-path preflight has invalid Adam state for {name}")
        nonzero = int(torch.count_nonzero(exp_avg))
        if nonzero == 0:
            raise RuntimeError(f"R054 active-path preflight produced zero gradient state for {name}")
        print(
            f"[r054] active_path={name} nonzero_adam_m={nonzero} "
...
if missing:
    raise RuntimeError(f"R054 active-path preflight is missing optimizer parameters: {missing}")
PY
}
...
postprocess() {
  ...
  [[ -s "$checkpoint" ]] || {
    echo "[r054] missing final checkpoint: $checkpoint" >&2
    exit 2
  }
  if [[ -e "$post_root" ]]; then
    echo "[r054] postprocess output already exists: $post_root" >&2
    exit 2
  fi
...
run_stage1 \
  "$PREFLIGHT_ID" \
  r054_primary_guide_shape_gaussian_residual_fullres_preflight.env
verify_active_path_preflight
touch "$CONTROL_ROOT/preflight_passed"
echo "[r054] preflight passed; waiting for $CONTROL_ROOT/authorize_run"
...
run_stage1 \
  "$RUN_ID" \
  r054_primary_guide_shape_gaussian_residual_0_30k.env
postprocess

touch "$CONTROL_ROOT/run_done"
echo "[r054] run and postprocess complete; waiting for $CONTROL_ROOT/release"
