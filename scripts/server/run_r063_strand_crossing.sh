...
PROJECT_ROOT="${PROJECT_ROOT:?set PROJECT_ROOT to the reviewed R063 checkout}"
RUNTIME_ROOT="${RUNTIME_ROOT:?set RUNTIME_ROOT to a new R063 runtime directory}"
...
EXPECTED_COMMIT="${EXPECTED_COMMIT:?set EXPECTED_COMMIT to the reviewed R063 commit}"
MESH_NO_PENETRATION_SDF="${MESH_NO_PENETRATION_SDF:?set the reviewed mesh SDF path}"

RUN_ID="${RUN_ID:-r063_strand_crossing_0_30k_h100_20260815}"
...
if [[ "$actual_commit" != "$EXPECTED_COMMIT" ]]; then
  echo "[r063] commit mismatch: expected=$EXPECTED_COMMIT actual=$actual_commit" >&2
  exit 2
fi
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "[r063] tracked checkout is dirty; refusing formal execution" >&2
  git status --short >&2
  exit 2
fi
if [[ -e "$OUTPUT_DIR" ]]; then
  echo "[r063] output already exists; refusing overwrite: $OUTPUT_DIR" >&2
  exit 2
fi
...
snapshot_config \
  "$PROJECT_ROOT/configs/r063_strand_crossing_0_30k.env" \
  "$CONTRACT_DIR/r063_0_30k.env"
...
delta = {
    key: {"r062": base.get(key), "r063": candidate.get(key)}
...
if unexpected or missing:
    raise RuntimeError(
        "R063 is not a strict crossing-only config: "
...
print("R063_CONFIG_DELTA_PASS")
PY

"$PYTHON" -m pytest \
  tests/test_strand_crossing_active_set.py \
  tests/test_stage1_strand_crossing.py \
  tests/test_stage1_no_penetration.py \
  tests/test_gaussian_rgb_residual.py \
  -q
...
echo "[r063] start formal run at=$(date --iso-8601=seconds) commit=$actual_commit"
...
CONFIG_PATH="$PROJECT_ROOT/configs/r063_strand_crossing_0_30k.env" \
RUN_PREFLIGHT=1 \
RUN_BATCH_PREFLIGHT=0 \
  bash "$PROJECT_ROOT/scripts/server/run_white_tiger_stage1.sh" \
  2>&1 | tee "$LOG_DIR/$RUN_ID.log"
echo "[r063] formal run complete at=$(date --iso-8601=seconds)"
