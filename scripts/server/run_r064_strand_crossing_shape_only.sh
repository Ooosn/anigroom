...
PROJECT_ROOT="${PROJECT_ROOT:?set PROJECT_ROOT to the reviewed R064 checkout}"
RUNTIME_ROOT="${RUNTIME_ROOT:?set RUNTIME_ROOT to a new R064 runtime directory}"
...
EXPECTED_COMMIT="${EXPECTED_COMMIT:?set EXPECTED_COMMIT to the reviewed R064 commit}"
MESH_NO_PENETRATION_SDF="${MESH_NO_PENETRATION_SDF:?set the reviewed mesh SDF path}"

RUN_ID="${RUN_ID:-r064_strand_crossing_shape_only_0_30k_h100_20260815}"
...
if [[ "$actual_commit" != "$EXPECTED_COMMIT" ]]; then
  echo "[r064] commit mismatch: expected=$EXPECTED_COMMIT actual=$actual_commit" >&2
  exit 2
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "[r064] checkout is dirty; refusing formal execution" >&2
  git status --short >&2
  exit 2
fi
if [[ -e "$OUTPUT_DIR" ]]; then
  echo "[r064] output already exists; refusing overwrite: $OUTPUT_DIR" >&2
  exit 2
fi
...
snapshot_config \
  "$PROJECT_ROOT/configs/r064_strand_crossing_shape_only_0_30k.env" \
  "$CONTRACT_DIR/r064_0_30k.env"
...
delta = {
    key: {"r062": base.get(key), "r064": candidate.get(key)}
...
if unexpected or missing:
    raise RuntimeError(
        "R064 is not a strict crossing-only config: "
...
echo "[r064] start formal run at=$(date --iso-8601=seconds) commit=$actual_commit"
...
CONFIG_PATH="$PROJECT_ROOT/configs/r064_strand_crossing_shape_only_0_30k.env" \
RUN_PREFLIGHT=1 \
RUN_BATCH_PREFLIGHT=0 \
  bash "$PROJECT_ROOT/scripts/server/run_white_tiger_stage1.sh" \
  2>&1 | tee "$LOG_DIR/$RUN_ID.log"
echo "[r064] formal run complete at=$(date --iso-8601=seconds)"
