...
CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_WEIGHT="${CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_WEIGHT:-0.0}"
CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_REDUCTION="${CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_REDUCTION:-mean_l1}"
...
VIEW_GATE_GEOMETRY_SUPPORT="${VIEW_GATE_GEOMETRY_SUPPORT:-0}"
VIEW_GATE_LENGTH_CONFIDENCE_SUPPORT="${VIEW_GATE_LENGTH_CONFIDENCE_SUPPORT:-0}"
...
cmd=(
  ...
  --clean-flow-guide-length-anchor-weight "$CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_WEIGHT"
  --clean-flow-guide-length-anchor-reduction "$CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_REDUCTION"
...
elif [[ "$CLEAN_FLOW_INIT" == "1" || "$CLEAN_FLOW_LENGTH_INIT" == "1" || "$CLEAN_FLOW_GUIDE_ANCHOR_WEIGHT" != "0" || "$CLEAN_FLOW_GUIDE_LENGTH_ANCHOR_WEIGHT" != "0.0" || "$VIEW_GATE_LENGTH_CONFIDENCE_SUPPORT" == "1" ]]; then
...
if [[ "$VIEW_GATE_GEOMETRY_SUPPORT" == "1" ]]; then
  cmd+=(--view-gate-geometry-support)
fi
if [[ "$VIEW_GATE_LENGTH_CONFIDENCE_SUPPORT" == "1" ]]; then
  cmd+=(--view-gate-length-confidence-support)
fi
